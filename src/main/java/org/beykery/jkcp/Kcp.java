...
public class Kcp {
	...
	public int peekSize() {
		...
		if (rcv_queue.size() < seq.frg + 1) { return -1; }
		int length = 0;
	...
	public int receive(ByteBuf buffer) {
		...
		if (len != peekSize) { throw new RuntimeException("数据异常."); }
	...
	public int send(ByteBuf buffer) {
		if (buffer.readableBytes() == 0) { return -1; }
		...
		if (this.stream && !this.snd_queue.isEmpty()) {
			...
			if (seg.data != null && seg.data.readableBytes() < mss) {
				...
				if (buffer.readableBytes() == 0) { return 0; }
			}
		}
		int count;
	...
	private int wnd_unused() {
		if (rcv_queue.size() < rcv_wnd) { return rcv_wnd - rcv_queue.size(); }
		return 0;
	}
